...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Devolvemos tanto la URL como el nombre del archivo para referencia
    return NextResponse.json({ url: publicUrlData.publicUrl, name: file.name }, { status: 200 });

  } catch (error) {
    console.error("Server-side news image upload error:", error);
